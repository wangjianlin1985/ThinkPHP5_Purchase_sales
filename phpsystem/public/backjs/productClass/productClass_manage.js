...
$(function () { 
	...
	$("#productClass_manage").datagrid({
		url : backURL + getVisitPath("ProductClass") + '/backList',
		fit : true,
		fitColumns : true,
		striped : true,
		rownumbers : true,
				...
				if ($("#productClassEditForm").form("validate")) {
					...
					if(!$("#productClassEditForm").form("validate")) {
						...
						$("#productClassEditForm").form({
						    url: backURL + getVisitPath("ProductClass") + "/update",
						    onSubmit: function(){
								if($("#productClassEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	productClass_manage_tool = {
		...
		exportExcel: function() {
			$("#productClassQueryForm").form({
			    url: backURL + getVisitPath("ProductClass") + "/outToExcel",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url :  backURL + getVisitPath("ProductClass") + "/deletes",
							data : {
